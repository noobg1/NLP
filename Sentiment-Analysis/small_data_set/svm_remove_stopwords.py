...
for p in short_pos.split('\n'):
    ...
    for w in pos:
        if w[1][0] in allowed_word_types and not w[1][0] in stopwords:
            all_words.append(("pos",w[0].lower()))
...
for p in short_neg.split('\n'):
    ...
    for w in pos:
        if w[1][0] in allowed_word_types and not w[1][0] in stopwords:
            all_words.append(( "neg",w[0].lower()))

for p in short_neutral.split('\n'):
    documents.append( ( "neu", p) )
    words = word_tokenize(p)
    pos = nltk.pos_tag(words)
    for w in pos:
        if w[1][0] in allowed_word_types and not w[1][0] in stopwords:
            all_words.append(( "neu",w[0].lower()))
...
if __name__ == "__main__":
        ...
        X, y ,T = create_tfidf_training_data(all_words,test_sen_all)
